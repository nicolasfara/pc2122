...
object Stochastics extends App {
  ...
  def draw[A](cumulativeList: List[(Double, A)])(implicit rnd: Random = random): A = {
  ...
  def statistics[A](choices: Set[(Double, A)], size: Int)(implicit rnd: Random = random): Map[A, Int] =
    (1 to size)
      .map(i => Stochastics.draw(cumulative(choices.toList))(random))
      .groupBy(identity)
      .view
      .mapValues(_.size)
      .toMap
